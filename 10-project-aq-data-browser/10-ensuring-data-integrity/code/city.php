...
<?php else: ?>
    ...
    <?php if (!empty($stats)): ?>
        ...
        <?php
        // Collect & sort the keys of the "$stats" array for the chart's "labels" property
        $labels = array_keys($stats);
        sort($labels);

        // Collect the PM25 data that will be assigned to the chart's "data" property (under "datasets")
        $pm25 = [];
        ...
        // Create the array for the Chart.js' datasets configuration
        $datasets = [];

        // Assign the corresponding configuration settings for "pm25" and "pm10" datasets
        if (array_sum($pm25) > 0):
            $datasets[] = [
